...
@patch("data.historical.requests.get")
def test_fetch_historical_air_quality_returns_empty_list_on_failure(mock_get):
    import requests
...
@patch("data.historical.requests.get")
def test_fetch_historical_weather_handles_short_array(mock_get):
    from data.historical import fetch_historical_weather

    # Mock response where one field array is shorter than time array
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "hourly": {
            "time": ["2026-05-01T00:00", "2026-05-01T01:00"],
            "temperature_2m": [28.1],  # Only 1 value, but time has 2
            "relative_humidity_2m": [60, 62],
            "apparent_temperature": [30.0, 29.2],
            "precipitation": [0.0, 0.0],
            "cloud_cover": [10, 15],
    ...
    assert len(records) == 2
    assert records[0]["temperature"] == 28.1
    assert records[1]["temperature"] is None  # Missing value returns None


@patch("data.historical.requests.get")
def test_fetch_historical_air_quality_handles_short_array(mock_get):
    from data.historical import fetch_historical_air_quality
    ...
    mock_response.json.return_value = {
        "hourly": {
            "time": ["2026-05-01T00:00", "2026-05-01T01:00"],
            "pm2_5": [80.0],  # Only 1 value, but time has 2
            "pm10": [130.0, 135.0],
            "carbon_monoxide": [450.0, 460.0],
            "nitrogen_dioxide": [30.0, 32.0],
            "sulphur_dioxide": [8.0, 9.0],
    ...
    assert len(records) == 2
    assert records[0]["pm25"] == 80.0
    assert records[1]["pm25"] is None  # Missing value returns None
